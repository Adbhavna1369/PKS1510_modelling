...
def load__plot_data(sed_path, title):
...
#import pandas as pd 
#import csv
#order of 2012, 2015a , 2015b, low_state
val_p1 =(1.8, 1.8,1.8,2.1)
val_p2 =(3.85, 3.5,3.5,4)
val_log10_ke = (0.2, 0.1 , 0.09,0.08)
val_log10_gammab =(900, 800,500,150)
val_log10_gammamax =(3e4,4e4, 3e4,3e4) 

list_val= [val_log10_ke, val_p1, val_log10_gammab, val_log10_gammamax]
for pars in list_val:
    print (pars)
dirs = os.listdir("/home/pc/Desktop/Thesis Datasets/")
count = 0
for d in dirs:
    if('ecsv' in d):
    
        count += 1
        sed_path = f"/home/pc/Desktop/Thesis Datasets/{d}"
        title = ( (sed_path.split("/")[-1]).split(".")[0]).split("_")[-1]
        print(sed_path)
        ...
        # - EED
        agnpy_ec.log10_k_e = np.log10(val_log10_ke)
        agnpy_ec.p1 = 2.1
        agnpy_ec.p2 = 4
        agnpy_ec.log10_gamma_b = np.log10(val_log10_gammab)
        agnpy_ec.log10_gamma_min = np.log10(1)
        agnpy_ec.log10_gamma_min.freeze()
        agnpy_ec.log10_gamma_max = np.log10(val_log10_gammamax)
